...
def _ensure_bucket_col(df):
    ...
    # GMC.com enforcement
    mask_gmc = (s.str.contains("gmc.com")) | (s.str.contains("gmc") & (s.str.contains("build your own") | s.str.contains("offer raq") | s.str.contains("locate vehicle") | s.str.contains("locate a vehicle") | s.str.contains("supplier discount")))
    ...
    # Canonical map first
    df["Bucket"]=df[lead].astype(str).apply(canonical_bucket)
    # Enforce GMC.com family roll-up
    s = df[lead].astype(str).str.lower().fillna("")
    mask_gmc = (s.str.contains("gmc.com") | ((s.str.contains(r"gmc")) & (s.str.contains("offer raq") | s.str.contains("build your own") | s.str.contains("locate vehicle"))))
    df.loc[mask_gmc,"Bucket"]="GMC.com"
    return df
# ===== end helpers =====

# Normalize cost inputs to bucket keys (e.g., any Cargurus variant -> Carguru)
try:
    if "cost_inputs" in st.session_state and isinstance(st.session_state["cost_inputs"], dict):
        st.session_state["cost_inputs"] = { canonical_bucket(k): v for k,v in st.session_state["cost_inputs"].items() }
except Exception:
    pass


# ===== Helper constants and functions (robust) =====
import csv, pandas as _pd, re as _re

# Non-internet markers (default internet unless matched)
NON_INTERNET_SUBSTRINGS = [
    "walk-in","walk in","phone","service","service dept","body shop","parts",
    "bdc outbound","referral","repeat","repeat customer","fleet","event","employee",
    "unknown","record journal","playbook manifest","hill car","drive by","drive-by","drive by - location"
...
# Paid buckets (used for agg rows)
PAID_BUCKETS = {
...
def _ensure_bucket_col(df):
    ...
    # Credit/Finance -> Credit Application
    df.loc[s.str.contains("credit") | s.str.contains("pre-qual") | s.str.contains("prequal") | s.str.contains("finance"), "Bucket"] = "Credit Application"
    # Carguru rollup
    df.loc[s.str.contains("guru"), "Bucket"] = "Carguru"
    return df
